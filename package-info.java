 ...
 * </PRE>
 * <P>
 *     A connection starts in the "start state;"  As shown in figure 1.  A connection can progress to the "general state"
 *             by receiving a start message; or it can transition to the new node state by receiving a new node message.
 ...
 * </P>
 * <P>
 *     An auction message causes the connection to enter the auction state.  The connection remains in that state through
 *             any number of bid messages before it receives an auction over message and returns to the general state.
 * </P>
 * <P>
 *     If the connection receives a start message then it has a configurable amount of time to send a start message of its
 *             own.  The connection then enters the general state.  This is the "ground state" for a connection and it can receive
 *             basically any message.  A timeout or error message causes it to re-enter the start state.
 * </P>
 * <P>
 *     A connection enters the new node state by receiving a new node message when in the start state.  The node then tells
 *             its new "partner" about all the messages it knows about by issuing a message message until it runs out of messages
 *             at which time it issues a new node over message and returns to the start state.
 * </P>
 * <P>
 * <PRE>
 *     &lt;dead node&gt; ::= DEAD &lt;UUID of dead node&gt;
 *             &lt;heart beat&gt; ::= HEARTBEET &lt;node UUID&gt;
 * </PRE>
 *         A node that doesn't respond back with a heart beat message of its own in a configurable period of time is considered
 *         dead.
 * </P>
 * <P>
 ...
 *             responds to the dead message then the connection reenters the general state.  It is suggested that the node that
 *             sent the dead message starts an auction immediately but this not a requirement.
 * </P>
 * 
 * <P>
 * <PRE>
 ...
 * 
 * <P>
 *     The integer values in the bid messages are expected to be random values that the nodes create for each bid.  If the
 *             node's value is
 *             greater than the other node's value then the node has "won" the message and it is added to that node's send queue.
 *             In the case of a tie, both nodes are expected to reissue bids immediately.  If another
 *             tie occurs then the nodes reissue bids until there is no tie. If the other node in the connection issues a bid for a
 *             message that the node does not have, then if the node wins the message then it should issue a get message message to
 *             get the status and delivery URLs and the message's contents, but this is not a requirement.
 * </P>
 * 
 * <P>
 * <PRE>
 ...
 *         node is required to issue one message message for each message it knows about.  When the node has exhausted all its
 ...
 *         reply.  When a node delivers a message it informs the other nodes via the message delivered message.  The other
 *         node makes no reply.
 * </P>
 * <P>
 * <PRE>
 ...
 *     &lt;timeout&gt; ::= TIMEOUT
 * </PRE>
 *         When a node receives a timeout message it signals that the other node in the connection has timed out waiting for a
 *         reply from the node.  Unless otherwise stated receiving a timeout message transfers the connection, in whatever
